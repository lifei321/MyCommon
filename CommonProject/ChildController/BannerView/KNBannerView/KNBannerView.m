...
- (void)initializeCollectionView{
    ...
    [layout setScrollDirection:self.scrollDirect];
    _layout = layout;
...
- (void)setScrollDirect:(UICollectionViewScrollDirection)scrollDirect {
    _scrollDirect = scrollDirect;
    _layout.scrollDirection = _scrollDirect;
}
...
- (void)timerRun{
    ...
    if (self.scrollDirect == UICollectionViewScrollDirectionVertical) {
       index = (_collectionView.contentOffset.y / _layout.itemSize.height) + 1;
    } else if (self.scrollDirect == UICollectionViewScrollDirectionHorizontal) {
        index = (_collectionView.contentOffset.x / _layout.itemSize.width) + 1;
    }
...
- (void)setIMGArray:(NSMutableArray *)IMGArray{
    _IMGArray = IMGArray;
    _imagesCount = IMGArray.count * 10; // extension array cout for enough cell to display
